            ...
            <div class="tmsg-commentshow">
                <ul class="tmsg-commentlist">
                    <li class="tmsg-c-item" v-for="(item, index) in commentList" :key="'common' + index">
                        <article class="">
                            <header>
                                <img :src="item.avatar?item.avatar:'static/img/default.jpg'"  @onerror="handleError">
                                <div class="i-name">
                                    {{ item.username }}
                                </div>
                                <div class="i-time">
                        ...
                        <ul v-show="item.children" class="tmsg-commentlist" style="padding-left:60px;">
                            <li class="tmsg-c-item" v-for="(citem, cindex) in item.children" :key="'citem' + cindex">
                                <article class="">
                                    <header>
                                        <img :src="citem.avatar?citem.avatar:'static/img/default.jpg'" @onerror="handleError">
                                        <div class="i-name">
...
import { sendComment, getArticleComment, getLinkComment, canComment } from '../api/comment.js'
...
export default {
    ...
    computed: {
            ...
            isComemnt: function() {
            console.log('ok')
            return canComment(that.that.$route.query.aid).then(result => {
                return result === "1"
            })
        },
    },
    methods: { //事件处理器
        ...
        // 错误头像url
        handleError() {  
            this.imageSrc = 'static/img/default.jpg';  
        },
        //选择表情包
